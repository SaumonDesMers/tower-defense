...
impl Plugin for SelectionPlugin {
	fn build(&self, app: &mut App) {
		app
			.add_observer(attach_selection_observers)
			// .add_systems(Update, log_on_selection_change)
			.insert_resource(Selection { entity: None, observer: None });
	}
}
...
fn attach_selection_observers(
	...
	selection.observer = Some(commands.spawn(Observer::new(select_entity).with_entities(selectables)).id());
}
...
fn select_entity(
	...
	if query.get(click.entity).is_ok() {
		if let Some(current) = selection.entity && current == click.entity {
